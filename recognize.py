...
# Load embeddings
embeddings = np.load(os.path.join(BASE_DIR, "embeddings.npy"), allow_pickle=True)
names = np.load(os.path.join(BASE_DIR, "names.npy"), allow_pickle=True)
...
embedder = cv2.dnn.readNetFromTorch(
    os.path.join(BASE_DIR, "models", "nn4.small2.v1.t7")
)
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.6:
            ...
            face_blob = cv2.dnn.blobFromImage(
                face, 1.0 / 255, (96, 96), (0, 0, 0), swapRB=True
            )

            embedder.setInput(face_blob)
            vec = embedder.forward()[0]
